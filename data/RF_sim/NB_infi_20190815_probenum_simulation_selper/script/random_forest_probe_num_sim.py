...
class run_random_forest:
	def __init__(self, tmp):
		...
		# for self.rep in range(0,numrep):
		...
		# 	a = RandomForestClassifier(n_estimators=estimates, n_jobs=-1, class_weight='balanced')
		...
		self.data_name = data_tmp+str(probe_num)+"_selper"
		if os.path.isdir(self.settings.result_dir+self.data_name) is False:
			os.makedirs(self.settings.result_dir+self.data_name)
		for self.rep in range(0,numrep):
			#X_train, X_test, y_train, y_test = train_test_split(self.arr.transpose(),  self.data_container.sample_category)
			print (self.rep,"th itaration, selper", probe_num)

			## それぞれに割り振ったサンプルのlistの場所を取ってくる。
			train_samples_pos,tune_samples_pos,test_samples_pos = self.data_container.split3()
			## とってきた場所をもとに、データとカテゴリーをとってくる
			X_train_tmp,y_train,X_tuning,y_tuning,X_test_tmp,y_test = self.data_container.data_pickup(train_samples_pos,tune_samples_pos,test_samples_pos)

			select = SelectPercentile(percentile=100*probe_num/452453).fit(X_train_tmp, y_train)
			X_train = select.transform(X_train_tmp)
			X_test = select.transform(X_test_tmp)
